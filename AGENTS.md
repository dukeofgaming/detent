...
- `src/` is for the CLI entry point (`main.rs`) and thin module declarations (`lib.rs`). No `.rs` files directly under `src/` except `main.rs` and `lib.rs` — all implementation lives in sub-modules (`src/compiler/`, `src/graph_validation/`) or `lib/` workspace crates. Module declarations under `src/` use `mod.rs` (`src/compiler/mod.rs`, `src/graph_validation/mod.rs`).
