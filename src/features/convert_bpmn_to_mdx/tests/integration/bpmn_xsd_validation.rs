...
fn test_valid_hello_world_files_pass_xsd_validation() {
    for (name, content) in hello_world_fixtures() {
        let result =
            detent::features::convert_bpmn_to_mdx::infrastructure::validate_bpmn_xsd(content);
        assert!(
            result.is_ok(),
...
fn test_invalid_element_fails_xsd_validation() {
    for (name, content) in hello_world_fixtures() {
        let invalid_bpmn = content
            ...
            .replace("</bpmn2:task>", "</bpmn2:notARealTask>");
        let result =
            detent::features::convert_bpmn_to_mdx::infrastructure::validate_bpmn_xsd(&invalid_bpmn);
        assert!(
            result.is_err(),
