...
fn when_flow_node_ids(world: &mut ConvertWorld) {
    ...
    let start = detent::features::bpmn_mdx_transpiler::adapters::bpmn::FlowNode::StartEvent(
        process.start_events[0].clone(),
    );
    let task =
        detent::features::bpmn_mdx_transpiler::adapters::bpmn::FlowNode::Task(process.tasks[0].clone());
    let end = detent::features::bpmn_mdx_transpiler::adapters::bpmn::FlowNode::EndEvent(
