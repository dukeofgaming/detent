...
fn when_compile(world: &mut ConvertWorld) {
    let defs = detent::features::bpmn_mdx_transpiler::application::compile::compile_to_definitions(
        &world.mdx_inputs,
    );
    match defs {
...
fn when_attempt_import(world: &mut ConvertWorld) {
    ...
    // Act
    match detent::features::bpmn_mdx_transpiler::adapters::bpmn::parse_bpmn(xml) {
        Ok(defs) => {
            // Act
            match detent::features::bpmn_mdx_transpiler::application::import::import_to_mdx(&defs) {
